/*
  ...
  url :
	...
	https://github.com/skulpt/skulpt/issues/695

  doc :
	https://github.com/skulpt/skulpt/wiki/Skulpt-Processing
...
var $builtinmodule = function (name) {
    ...
    mod.run = new Sk.builtin.func(function () {
        function sketchProc (processing) {
            ...
            if (Processing.instances.length >= 2) {
                Processing.instances.shift().noLoop();
            }
    ...
    keyboardClass = function ($gbl, $loc) {
	    ...
	    key = Sk.ffi.remapToJs(key);
            if (key === "key") {
                try {
                    if (mod.processing.key.valueOf() == 0xffff)
                        return mod.CODED;
                    return new Sk.builtin.str(mod.processing.key.toString());
                } catch (e) {
                    return new Sk.builtin.str("\0");
                }
            }
            else if (key === "keyCode") {
